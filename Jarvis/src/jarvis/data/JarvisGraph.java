...
public class JarvisGraph {
	...
	public JarvisNode getNode(String data) {
		if (m_database.connect()) {	
			try {
				...
				if (rs.next()) {
					JarvisNode entry = new JarvisNode(rs.getInt("node_id"), rs.getString("node_type"), rs.getString("node_data"));
					return entry;
				}
	...
	public List<JarvisNode> getNeighbors(JarvisNode node) {
		if (m_database.connect()) {
			try {
				String query = "SELECT n2.node_id, n2.node_type, n2.node_data FROM nodes INNER JOIN edges ON nodes.node_id = edges.edge_n1 OR nodes.node_id = edges.edge_n2 INNER JOIN nodes AS n2 ON ((edges.edge_n1 = n2.node_id OR edges.edge_n2 = n2.node_id) AND n2.node_data <> ?) WHERE nodes.node_data = ?;";
				PreparedStatement statement = m_database.getConnection().prepareStatement(query);
				...
				while (rs.next()) {
					JarvisNode n = new JarvisNode(rs.getInt("node_id"), rs.getString("node_type"), rs.getString("node_data"));
					nodes.add(n);
				}
				return nodes;
			} catch (SQLException e) {
	...
	public JarvisNode addNode(String type, String data) {
		if (m_database.connect()) {
			try {
				String query = "INSERT INTO nodes(node_type, node_data) VALUES (?, ?)";
				...
				if (rs.next()) {
					...
					JarvisNode newNode = new JarvisNode(id, type, data);
					return newNode;
				}
			} catch (SQLException e) {
				System.err.println(e.getMessage());
	...
	public JarvisEdge addEdge(JarvisNode n1, JarvisNode n2) {
		if (m_database.connect()) {
			try {
				String query = "INSERT INTO edges(edge_n1, edge_n2) VALUES (?, ?)";
				PreparedStatement statement = m_database.getConnection().prepareStatement(query, PreparedStatement.RETURN_GENERATED_KEYS);
				statement.setInt(1, n1.getNodeId());
				statement.setInt(2, n2.getNodeId());
				statement.executeUpdate();
				...
				if (rs.next()) {
					...
					JarvisEdge newEdge = new JarvisEdge(id, n1.getNodeId(), n2.getNodeId());
					return newEdge;
				}
			} catch (SQLException e) {
				System.err.println(e.getMessage());
